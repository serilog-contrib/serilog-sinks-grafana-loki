...
let private writeThenFlush (lokiUri: string) (configure: LokiSinkOptions -> LokiSinkOptions) events = task {
    let options =
        LokiSinkOptions.Defaults
        |> fun o -> { o with Uri = lokiUri }
        |> configure
...
type LokiIntegrationTests(loki: LokiFixture) =
    ...
    let testLabel key = [| { Key = "testrun"; Value = runId }; { Key = "test"; Value = key } |]
    ...
    [<Fact>]
    member _.``e2e: log event is queryable in Loki after flush`` () : Task = task {
        let startNs = nowNs ()
        let selector = $"{{testrun=\"{runId}\",test=\"e2e-basic\"}}"
        do! writeThenFlush loki.Uri
                (fun o -> { o with Labels = testLabel "e2e-basic" })
    ...
    [<Fact>]
    member _.``labels: stream is reachable via label selector`` () : Task = task {
        let startNs = nowNs ()
        let appVal  = $"app-{runId}"
        let selector = $"{{app=\"{appVal}\"}}"
        do! writeThenFlush loki.Uri
                (fun o -> { o with Labels = [| { Key = "app"; Value = appVal } |] })
    ...
    [<Fact>]
    member _.``body: log line body is valid JSON with Message field`` () : Task = task {
        let startNs = nowNs ()
        let selector = $"{{testrun=\"{runId}\",test=\"body-roundtrip\"}}"
        do! writeThenFlush loki.Uri
                (fun o -> { o with Labels = testLabel "body-roundtrip" })
    ...
    [<Fact>]
    member _.``body: exception field present after roundtrip`` () : Task = task {
        let startNs = nowNs ()
        ...
        do! writeThenFlush loki.Uri
                (fun o -> { o with Labels = testLabel "exception-roundtrip" })
                [ event ]
        let! entries = waitForLogs loki.Uri selector startNs
        test <@ entries.Length = 1 @>
        use body = JsonDocument.Parse(entries[0].Line)
    ...
    [<Fact>]
    member _.``streaming: two consecutive flushes both appear in Loki`` () : Task = task {
        let startNs = nowNs ()
        let selector = $"{{testrun=\"{runId}\",test=\"two-batches\"}}"
        let labels   = testLabel "two-batches"
        do! writeThenFlush loki.Uri (fun o -> { o with Labels = labels })
    ...
    [<Fact>]
    member _.``labels: Fatal level stored as "fatal" (V9 change from "critical")`` () : Task = task {
        let startNs  = nowNs ()
        let selector = $"{{testrun=\"{runId}\",test=\"fatal-label\",level=\"fatal\"}}"
        do! writeThenFlush loki.Uri
                (fun o -> { o with Labels = testLabel "fatal-label" })
    ...
    [<Fact>]
    member _.``throughput: batch of 200 events all arrive in Loki`` () : Task = task {
        let startNs  = nowNs ()
        let selector = $"{{testrun=\"{runId}\",test=\"large-batch\"}}"
        let events   =
            [ for i in 1 .. 200 ->
                mkEvent LogEventLevel.Information $"event {i}" [] ]
        do! writeThenFlush loki.Uri
                (fun o -> { o with Labels = testLabel "large-batch" })
                events
        let! entries = waitForLogs loki.Uri selector startNs
    ...
    [<Fact>]
    member _.``gzip: compressed request accepted by Loki`` () : Task = task {
        let startNs  = nowNs ()
...
and GzipRequestHandler(inner: HttpMessageHandler) =
    ...
    override self.SendAsync(request, ct) = task {
