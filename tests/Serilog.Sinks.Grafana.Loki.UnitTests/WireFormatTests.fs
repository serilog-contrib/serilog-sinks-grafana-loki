...
type FakeHttpHandler() =
    ...
    member _.LastBodyJson =
        JsonDocument.Parse(captured[captured.Count - 1].BodyBytes)
...
let private makeSink (configure: LokiSinkOptions -> LokiSinkOptions) =
    ...
    let options =
        ...
        |> fun o -> { o with Uri = "http://localhost:3100"; HttpClient = client }
        |> configure
...
let private makeSinkWithHandler (configure: LokiSinkOptions -> LokiSinkOptions) =
    ...
    let options =
        LokiSinkOptions.Defaults
        |> fun o -> { o with Uri = "http://localhost:3100"; HttpMessageHandler = handler }
        |> configure
    let sink = new LokiSink(options)
    handler, sink

let private flush (sink: LokiSink) (events: LogEvent list) = task {
    do! (sink :> IBatchedLogEventSink).EmitBatchAsync(events)
}

// JSON navigation — all return plain values so they're safe inside test <@ ... @>
let private streamCount  (doc: JsonDocument) = doc.RootElement.GetProperty("streams").GetArrayLength()
let private streamAt i   (doc: JsonDocument) = doc.RootElement.GetProperty("streams")[i]
let private labelOf (key: string) (stream: JsonElement) =
    match stream.GetProperty("stream").TryGetProperty(key) with
    | true, v -> Some (v.GetString()) | _ -> None
let private hasLabel (key: string) (stream: JsonElement) =
    stream.GetProperty("stream").TryGetProperty(key) |> fst
let private valueCount   (stream: JsonElement) = stream.GetProperty("values").GetArrayLength()
...
[<Fact>]
let ``json structure: single event produces one stream`` () : Task = task {
...
[<Fact>]
let ``json structure: stream has one values entry for one event`` () : Task = task {
    let handler, sink = makeSink id
    use _ = sink
    do! flush sink [ mkInfo [] ]
    use doc = handler.LastBodyJson
...
[<Fact>]
let ``json structure: timestamp is a numeric nanosecond string`` () : Task = task {
    let handler, sink = makeSink id
    use _ = sink
    do! flush sink [ mkInfo [] ]
    use doc       = handler.LastBodyJson
...
[<Fact>]
let ``json structure: body is a valid JSON object`` () : Task = task {
    let handler, sink = makeSink id
    use _ = sink
    do! flush sink [ mkInfo [] ]
    use doc      = handler.LastBodyJson
...
[<Fact>]
let ``body: contains Message and MessageTemplate`` () : Task = task {
    let handler, sink = makeSink id
    use _ = sink
    do! flush sink [ mkInfo [] ]
    use doc = handler.LastBodyJson
...
[<Fact>]
let ``body: reserved property "Message" sanitized to "_Message"`` () : Task = task {
    let handler, sink = makeSink id
    use _ = sink
...
[<Fact>]
let ``body: Exception field present when event has exception`` () : Task = task {
    let handler, sink = makeSink id
    use _ = sink
...
[<Fact>]
let ``level label: present by default (HandleLogLevelAsLabel = true)`` () : Task = task {
    let handler, sink = makeSink id
    use _ = sink
    do! flush sink [ mkInfo [] ]
    use doc = handler.LastBodyJson
...
[<Fact>]
let ``level label: absent when HandleLogLevelAsLabel = false`` () : Task = task {
...
[<Fact>]
let ``level label: Fatal maps to "fatal" — V9 change from V8 "critical"`` () : Task = task {
    let handler, sink = makeSink id
    use _ = sink
...
[<InlineData(4, "error")>]
[<InlineData(5, "fatal")>]
let ``level label: all six Serilog levels map to Grafana vocabulary``
    (levelInt: int) (expected: string) : Task = task {
...
[<Fact>]
let ``labels: global labels appear in stream labels`` () : Task = task {
...
[<Fact>]
let ``labels: property promoted to label appears in stream`` () : Task = task {
    let handler, sink = makeSink (fun o -> { o with PropertiesAsLabels = [| "RequestPath" |] })
    use _ = sink
...
[<Fact>]
let ``labels: promoted property ALSO stays in body (V9 — V8 removed it by default)`` () : Task = task {
    let handler, sink = makeSink (fun o -> { o with PropertiesAsLabels = [| "RequestPath" |] })
    use _ = sink
    do! flush sink [ mkInfo [ "RequestPath", box "/health" ] ]
    use doc = handler.LastBodyJson
...
[<Fact>]
let ``labels: global label wins over property with same key`` () : Task = task {
    let globals = [| { Key = "app"; Value = "global-value" } |]
    let handler, sink =
        makeSink (fun o ->
            { o with Labels = globals; PropertiesAsLabels = [| "app" |] })
    use _ = sink
...
[<Fact>]
let ``labels: numeric property key gets param prefix`` () : Task = task {
    let handler, sink = makeSink (fun o -> { o with PropertiesAsLabels = [| "0" |] })
    use _ = sink
...
[<Fact>]
let ``grouping: events with different label values produce separate streams`` () : Task = task {
    let handler, sink = makeSink (fun o -> { o with PropertiesAsLabels = [| "env" |] })
    use _ = sink
    do! flush sink [
        mkInfo [ "env", box "prod" ]
        mkInfo [ "env", box "dev"  ]
...
[<Fact>]
let ``grouping: events with same labels group into one stream`` () : Task = task {
    let handler, sink = makeSink id
    use _ = sink
...
[<Fact>]
let ``http: request targets /loki/api/v1/push`` () : Task = task {
...
[<Fact>]
let ``http auth: Authorization header set when credentials provided`` () : Task = task {
...
[<Fact>]
let ``http auth: no Authorization header when no credentials`` () : Task = task {
...
[<Fact>]
let ``http tenant: X-Scope-OrgID header set when Tenant configured`` () : Task = task {
